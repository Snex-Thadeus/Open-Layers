...
var stateLayerName = 'ind_st';
var districtLayerName = 'ind_ds';
var cityLayerName = 'ind_ct';
var roadLayerName = 'india_roads';
var wbLayerName = 'india_waterbody';
var bldLayerName = 'india_bld';
var identifyLayers = [];
...
var indiaBldTile = new ol.layer.Image({
    title: "India Buildings",
    source: new ol.source.ImageWMS({
        url: 'http://' + serverPort + '/geoserver/' + geoserverWorkspace + '/wms',
        params: { 'LAYERS': geoserverWorkspace + ':' + bldLayerName, 'TILED': true },
        serverType: 'geoserver',
        visible: false
    })
});

var indiaWbTile = new ol.layer.Tile({
    title: "India Waterbodies",
    source: new ol.source.TileWMS({
        url: 'http://' + serverPort + '/geoserver/' + geoserverWorkspace + '/wms',
        params: { 'LAYERS': geoserverWorkspace + ':' + wbLayerName, 'TILED': true },
        serverType: 'geoserver',
        visible: false
    })
});
...
var indiaRdTile = new ol.layer.Tile({
    ...
    source: new ol.source.TileWMS({
        url: 'http://' + serverPort + '/geoserver/' + geoserverWorkspace + '/wms',
        params: { 'LAYERS': geoserverWorkspace + ':' + roadLayerName, 'TILED': true },
        serverType: 'geoserver',
        visible: false
    })
});

var indiaCtTile = new ol.layer.Tile({
    title: "India Cities",
    source: new ol.source.TileWMS({
        url: 'http://' + serverPort + '/geoserver/' + geoserverWorkspace + '/wms',
        params: { 'LAYERS': geoserverWorkspace + ':' + cityLayerName, 'TILED': true },
        serverType: 'geoserver',
        visible: false
    })
});

var indiaDsTile = new ol.layer.Image({
    title: "India Districts",
    source: new ol.source.ImageWMS({
        url: 'http://' + serverPort + '/geoserver/' + geoserverWorkspace + '/wms',
        params: { 'LAYERS': geoserverWorkspace + ':' + districtLayerName, 'TILED': true },
        serverType: 'geoserver',
        visible: true
    })
});
...
function createRows(data, layerName) {
    ...
    for (var key in data) {
        ...
        for (var key2 in data2) {
            ...
            else if (layerName == districtLayerName) { td2.setAttribute('onClick', 'zoomToFeature(this,\'' + districtLayerName + '\',\'' + key2 + '\')'); }
            else {  }
        }
...
function zoomToFeature(featureName, layerName, attributeName) {
    ...
    var url = "http://localhost:8080/geoserver/GIS/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=" + value_attribute + "+" + value_operator + "+'" + value_txt + "'&outputFormat=application/json"
    // console.log(url);
